...
//				}
				...
				switch ( $pAction ) {
					...
					case "doQuery" :
						...
						if ( $query === false ) {
							...
							if ( $result !== false ) {
								...
								$header .= '<span class="uk-text-warning">' . htmlspecialchars( $query );
								$header .= '</span></p>';
					...
					case "doQueryNS":
						...
						$header .= '<p><span class="uk-text-warning">';
						$header .= wfMessage( 'wsps-special_custom_ns_query', $nsName )->text();
						if ( $startsWith ) {
				...
				$out->addHTML(
					'<div class="uk-child-width-1-2@m uk-grid-divider uk-grid-medium uk-grid-match" uk-grid>'
				);
				$SMWQueryDisabledText = '';
				if ( !$specialSMW->isExtensionInstalled( 'SemanticMediaWiki' ) ) {
					$SMWQueryDisabledText = self::makeAlert(
						wfMessage( 'wsps-special_custom_query_we_need_smw' )->text()
					);
				}
